<script type='text/javascript'> 
  ...
  function imprimir_pdf() 
  {
    ...
    var auxcss = '<link rel="stylesheet" type="text/css" href="cadastro.css">';

    //1ª Alternativa para impressão até funcionar: window.print(nomearqcompleto);

    var conteudo = document.getElementById('campos').innerHTML;

    paraSalvar = window.open(nomearqcompleto, 'about: _blank','width=800','height=600');//window.print

    paraSalvar.document.write('<html><head>' + auxcss + '<title>' + nomearquivo + '</title></head><body>'); 
    paraSalvar.document.write(conteudo);
    paraSalvar.document.write('</body></html>');

    paraSalvar.window.print();
    paraSalvar.window.close();

    //paraSalvar.window.fliename(nomearquivo);
    //system('PDFToPrinter.exe', nomearqcompleto);
  }
  //*/
</script>
...
<script type='text/javascript'>
  ...
  {  
    ...
    //Todas as funções funcionaram
    alert("Função SavarPDF concluída");
  }
  //*/
</script>
...
<script type='text/javascript'>
  $(document).ready(function()
  {
    $("input[name='numero_chamado']").show(function()
    {
      ...
      $.getJSON('autopreenCliente.php', {numero_chamado: $(this).val()}, function(json)
      {       
  ...
  //Todas as funções funcionaram
  //alert("Leitura e pré-carregamento do coumento foram concluídas");
