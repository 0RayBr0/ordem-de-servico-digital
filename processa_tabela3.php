<?php include_once "conexao.php";
	...
	//header('Content-Type: text/html; charset=utf-8');
	     ...
	     //criação de variável '$' . Refere-se ao campo input  ,ja existente ,por meio do metodo POST inserido no "cadastro.php"
	    //filter_input — Obtem a específica variável externa pelo nome e  a filtra
	...
	$avaliacao_do_servico_executado= filter_input(INPUT_POST, 'radio_AVL_SERVICO' , FILTER_SANITIZE_STRING);
	...
	if($data_abertura_osc == '')	$data_abertura_osc_aux = 'Default'; else $data_abertura_osc_aux = "'".$data_abertura_osc."'";

	
	if($data_visto_saida == ''){
		$data_visto_saida = date("Y-m-d");
		$data_visto_saida_aux = "'".$data_visto_saida."'";
	}
	else $data_visto_saida_aux = "'".$data_visto_saida."'";
	...
	if($hora_abertura_osc == '') 	$hora_abertura_osc_aux = "'".$hora_abertura_osc."'"; else $hora_abertura_osc_aux = "'".$hora_abertura_osc."'";


	if($hora_visto_saida == ''){
		$hora_visto_saida = date("H:i"); 
		$hora_visto_saida_aux = "'".$hora_visto_saida."'"; 
	}
	else $hora_visto_saida_aux = "'".$hora_visto_saida."'";
	...
	$sql_cabecalho =  "INSERT INTO table3_osc VALUES (
	Default, 
	'$numero_chamado', 
	'$cliente', 
	'$mantenedores', 
	...
	//é preciso respeitar a ordem dos valores a serem inseridos  de acordo com os campos da osc
...
use PHPMailer\PHPMailer\Exception;
  ...
  try { if(mysqli_query($conn, $sql_cabecalho)){

	echo "<script>alert('Dados salvos!'); window.location = 'OSC_Cliente.php'; </script>"; 
 ...
 //$mail->SMTPDebug = SMTP::DEBUG_SERVER;                      //Enable verbose debug output
  $mail->CharSet = 'UTF-8';
    ...
    $mail->Username   = 'rayanne@centaurotelecom.com.br';                     //SMTP username
    $mail->Password   = '25560123';                               //SMTP password
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
    $mail->Port       = 587;                                    //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

     $mail->setFrom('rayanne@centaurotelecom.com.br', 'Ordem de Serviço');
    $mail->addAddress('raybretas@gmail.com', 'Rayanne');     //Add a recipient
             /*  $mail->addAddress('leonardo@centaurotelecom.com.br', 'Leonardo'); */
    
    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = ' Avaliação enviada com sucesso!';
    $mail->Body    = ' <br> Visualizar Ordem de Serviço completa :http://www2.centaurotelecom.com.br:8080/formulario2/OSC_CompletaTrancada.php<br>';
    $mail->AltBody = '';
    ...
    echo "Erro ao enviar. Error PHPMailer: {$mail->ErrorInfo}";
